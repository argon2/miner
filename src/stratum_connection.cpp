    ...
    )
{
    try
    {
        ...
        if (id == "4")
        {
            ...
            /**
             * If the rejection was caused by a low difficulty share increase
             * the difficulty_factor.
             */
            if (
                error_message.find("low difficulty share") != std::string::npos
                )
            {
                difficulty_factor_ = (difficulty_factor_ * 2.0) / 3.0;
                
                log_info(
                    "Stratum connection reduced difficulty factor to " <<
                    difficulty_factor_ << "."
                );
            }
        }
    ...
    )
{
    ...
    if (clean_jobs)
    {
         ...
         */
        stack_impl_.get_work_manager()->set_stratum_work(nullptr);
    }
    ...
    if (work)
    {
         ...
         */
        stack_impl_.get_work_manager()->set_stratum_work(work);
    }
}
    ...
    )
{
     ...
     * Set the difficulty.
     */
    auto difficulty = m_next_difficulty * difficulty_factor_;
